...
def embedder():
    try:
        ...
        ):  # Should be caught by MultimodalEmbedder's _load_model
            pytest.skip(
...
def test_load_image_from_url(embedder):
    ...
    image_url = (
        "https://via.placeholder.com/150/FF0000/FFFFFF?Text=TestImage"  # Red square
    )
    try:
...
def test_multimodal_embedder_invalid_model_name():
    with pytest.raises(
        Exception
    ):  # Expecting some form of error, could be more specific
