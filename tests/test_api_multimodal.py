...
def test_multimodal_embed_image_url(client):
    ...
    image_url = (
        "https://via.placeholder.com/150/0000FF/FFFFFF?Text=TestAPIImage"  # Blue square
    )
    ...
    try:
        ...
        Exception
    ) as e:  # Catch potential network errors during test execution if placeholder is down
        pytest.skip(
...
def test_multimodal_embed_invalid_image_url(client):
    ...
    )  # Unprocessable Entity for Pydantic validation errors on Form data
...
def test_multimodal_no_api_key(client):
    ...
    )  # Unauthorized or however your get_api_key handles it
